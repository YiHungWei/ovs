...
/*
XXX: TO_REMOVE
#define CT_DPIF_TP_ATTRS \
    CT_DPIF_TP_ATTR(TCP_SYN_SENT) \
    CT_DPIF_TP_ATTR(TCP_SYN_RECV) \
    CT_DPIF_TP_ATTR(TCP_EST) \
    CT_DPIF_TP_ATTR(TCP_FIN_WAIT) \
    CT_DPIF_TP_ATTR(TCP_CLOSE_WAIT) \
    CT_DPIF_TP_ATTR(TCP_LAST_ACK) \
    CT_DPIF_TP_ATTR(TCP_TIME_WAIT) \
    CT_DPIF_TP_ATTR(TCP_CLOSE) \
    CT_DPIF_TP_ATTR(TCP_SYN_SENT2) \
    CT_DPIF_TP_ATTR(TCP_RETRANS) \
    CT_DPIF_TP_ATTR(TCP_UNACK) \
    CT_DPIF_TP_ATTR(UDP_FIRST) \
    CT_DPIF_TP_ATTR(UDP_SINGLE) \
    CT_DPIF_TP_ATTR(UDP_MULTIPLE) \
    CT_DPIF_TP_ATTR(ICMP_FIRST) \
    CT_DPIF_TP_ATTR(ICMP_REPLY)


enum OVS_PACKED_ENUM ct_dpif_tp_attr {
#define CT_DPIF_TP_ATTR(ATTR) CT_DPIF_TP_ATTR_##ATTR,
    CT_DPIF_TP_ATTRS
...
struct ct_dpif_timeout_policy {
    uint32_t    id;
    uint32_t    attrs[CT_DPIF_TP_ATTR_MAX];     /* An array of timeout
                                                 * attribute values */
    uint32_t    present;                        /* If a timeout attribute is
                                                 * present set the
                                                 * corresponding bit. */
    struct ovs_list node;
};
