...
/* Each time this timer expires, the bridge tries to delete all the timeout
    ...
    for (i = 0; i < cfg->n_datapaths; i++) {
        ...
        HMAP_FOR_EACH (br, node, &all_bridges) {
            if (!strcmp(br->type, key)) {
                if (br->ofproto->ofproto_class->ct_zone_timeout_policy_reconfig) {
                    br->ofproto->ofproto_class->ct_zone_timeout_policy_reconfig(
                        br->ofproto, dp_cfg, idl_seqno);
                }
                break;
            }
        }
...
static void
ct_zone_timeout_policy_sweep(void)
{
    struct bridge *br;

    if (time_msec() >= ct_zone_timeout_policy_timer) {
        HMAP_FOR_EACH (br, node, &all_bridges) {
            if (br->ofproto->ofproto_class->ct_zone_timeout_policy_sweep) {
                br->ofproto->ofproto_class->ct_zone_timeout_policy_sweep(br->ofproto);
            }
        }
        ct_zone_timeout_policy_timer = time_msec() + CT_ZONE_TIMEOUT_POLICY_TIMER_INTERVAL;
    }
}
