...
AC_DEFUN([OVS_CHECK_LINUX], [
  ...
  if test -n "$KBUILD"; then
    ...
    if test "$version" -ge 5; then
       if test "$version" = 5 && test "$patchlevel" -le 3; then
          : # Linux 5.x
       else
          AC_ERROR([Linux kernel in $KBUILD is version $kversion, but version newer than 5.3.x is not supported (please refer to the FAQ for advice)])
       fi
