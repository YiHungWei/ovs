...
static void
ct_zone_destroy(struct dpif_backer *backer, struct ct_zone *zone)
{
...
static void
ct_timeout_policy_destroy(struct dpif_backer *backer, struct ct_timeout_policy *tp)
{
    cmap_remove(&backer->ct_tps, &tp->node, uuid_hash(&tp->uuid));
    ct_dpif_del_timeout_policy(backer->dpif, tp->cdtp.id);
...
init_ct_zone_timeout_policy(struct dpif_backer *backer)
{
    ...
    while (!(err = ct_dpif_timeout_policy_dump_next(backer->dpif, state, &cdtp))) {
        tp = xzalloc(sizeof *tp);
        tp->cdtp = cdtp;
...
ct_zone_timeout_policy_revalidate(struct dpif_backer *backer)
{
    ...
    CMAP_FOR_EACH (zone, node, &backer->ct_zones) {
        if (zone->last_used_seqno != backer->last_idl_change) {
            VLOG_ERR("Delete zone %"PRIu16, zone->id);
            ct_zone_destroy(backer, zone);
        }
    }

    CMAP_FOR_EACH (tp, node, &backer->ct_tps) {
        if (tp->last_used_seqno != backer->last_idl_change) {
            ...
            VLOG_ERR("Remove timeout policy id = %"PRIu32, tp->cdtp.id);
        }
    }
}

static void
ct_zone_timeout_policy_reconfig(const struct ofproto *ofproto_,
                                ...
                                unsigned int idl_seqno)
{
    ...
    backer->last_idl_change = idl_seqno;
    ...
    for (i = 0; i < dp_cfg->n_ct_zones; i++) {
        ...
        if (!tp) {
            ...
            if (tp) {
                ...
                ct_dpif_add_timeout_policy(backer->dpif, &tp->cdtp);
            }
        } else {
    ...
    ct_zone_timeout_policy_revalidate(backer);
}

static void
ct_zone_timeout_policy_sweep(const struct ofproto *ofproto_)
{
    struct ofproto_dpif *ofproto = ofproto_dpif_cast(ofproto_);
    struct dpif_backer *backer = ofproto->backer;
    struct ct_timeout_policy *tp;
    ...
    LIST_FOR_EACH (tp, list_node, &backer->ct_tp_kill_list) {
        ...
        if (!err) {
            ...
            VLOG_INFO_RL(&rl, "Failed to delete timeout policy id = %"PRIu32
                         " %s", tp->cdtp.id, ovs_strerror(err));
        }
    }
}
