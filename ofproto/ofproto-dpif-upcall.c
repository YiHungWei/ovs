    ...
    case CONTROLLER_UPCALL:
        {
            ...
            *am = (struct ofproto_async_msg) {
            ...
            if (flow->tunnel.flags & FLOW_TNL_F_UDPIF) {
                const struct tun_table *tun_tab = ofproto_get_tun_tab(&upcall->ofproto->up);
                int err = tun_metadata_from_geneve_udpif(tun_tab, &flow->tunnel,
                        &flow->tunnel, &frozen_flow.tunnel);
                if (err) {
                    VLOG_INFO("geneve tunnel metatadata error\n");
                }
            }
